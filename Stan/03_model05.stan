data {
  ...
  vector[nItems-1] priorMeanBetweenLambda;     // prior mean for item discrimination/loadings
  matrix[nItems-1, nItems-1] priorCovBetweenLambda;  // prior covariance matrix for item discrimination/loadings
...
parameters {
  ...
  vector[nItems-1] betweenLambda;                    // item discrimination/loadings
...
model {
  ...
  betweenLambda ~ multi_normal(priorMeanBetweenLambda, priorCovBetweenLambda);  // Prior for item discrimination/loadings
  ...
  for (person in 1:nPersons){
      ...
      Y[1, person] ~ bernoulli_logit(gamma[1] + personTheta[person] + 
                                        betweenLambda[1]*schoolTheta[school[person]] + itemRI[school[person], 1]);
      for (item in 2:(nItems-1)){
          Y[item, person] ~ bernoulli_logit(gamma[item] + withinLambda[item-1]*personTheta[person] +
                                        betweenLambda[item]*schoolTheta[school[person]] + itemRI[school[person], item]);
      }
      // last item has betweenLambda =1
      Y[nItems, person] ~ bernoulli_logit(gamma[nItems] + withinLambda[nItems-1]*personTheta[person] +
                                        schoolTheta[school[person]] + itemRI[school[person], nItems]);
...
generated quantities {
  ...
  for (person in 1:nPersons){
    ...
    for (item in 2:(nItems-1)){    
      ...
      personLike[person] = 
        ...
        bernoulli_logit_lpmf(Y[item, person] | 
                                gamma[item] + withinLambda[item-1]*personTheta[person] +
                                betweenLambda[item]*schoolTheta[school[person]] + itemRI[school[person], item]);
    ...
    personLike[person] = 
        personLike[person] + 
        bernoulli_logit_lpmf(Y[nItems, person] | 
                                gamma[nItems] + withinLambda[nItems-1]*personTheta[person] +
                                schoolTheta[school[person]] + itemRI[school[person], nItems]); 
    
    // generate data based on distribution and model
    simY[nItems, person] = bernoulli_logit_rng(
       gamma[nItems] + withinLambda[nItems-1]*personTheta[person] +
       schoolTheta[school[person]] + itemRI[school[person], nItems]
    );
  }
