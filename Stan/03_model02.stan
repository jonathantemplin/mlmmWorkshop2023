...
model {
  ...
  for (person in 1:nPersons){
      for (item in 1:nItems){
          Y[item, person] ~ bernoulli_logit(gamma[item] + personTheta[person] + itemRI[school[person], item]);
      }
  }
...
generated quantities {
  ...
  for (person in 1:nPersons){
    for (item in 1:nItems){    
      ...
      personLike[person] = 
        personLike[person] + 
        bernoulli_logit_lpmf(Y[item, person] | gamma[item] + personTheta[person] + itemRI[school[person], item]);

      // generate data based on distribution and model
      simY[item, person] = bernoulli_logit_rng(gamma[item] + personTheta[person] + itemRI[school[person], item]);
    }
  }
